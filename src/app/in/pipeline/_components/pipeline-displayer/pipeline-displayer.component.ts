...
export class PipelineDisplayerComponent {
    ...
    public source: Node | undefined;

    @Output() addLink = new EventEmitter<{ source: Node, target: Node }>();
    ...
    linkTo(node: Node) {
        if (this.source !== undefined && this.source !== node) {
            this.addLink.emit({ source: this.source, target: node });
