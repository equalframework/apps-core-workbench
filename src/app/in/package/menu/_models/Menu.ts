...
export class MenuElement {
    ...
    public id_compliancy(id_list:string[]):{ok:boolean,id_list:string[]} {
...
export class MenuItem extends MenuElement {
    ...
    constructor(schema:any = {}) {
        ...
        if(schema.id) {
            this.id = schema.id;delete schema.id;
        }
        if(schema.label) {
            this.label = schema.label;delete schema.label;
        }
        if(schema.icon) {
            this.icon = schema.icon;delete schema.icon;
        }
        if(schema.type) {
            this.type = schema.type;delete schema.type;
        }
        if(schema.description) {
            this.description = schema.description;delete schema.description;
        }
        if(schema.route) {
            this.route = schema.route;delete schema.route;
        }
...
export class MenuContext extends MenuElement {
    ...
    constructor(schema:any = {}) {
        ...
        if(schema.display_mode) {
            this.display_mode = schema.display_mode;
    ...
    override export() {
        ...
        if(this.domain && this._has_domain) {
            result.domain = this.domain;
        }
        if(this.sort && this._has_sort) {
            result.sort = this.sort;
        }
        if(this.order && this._has_order) {
        ...
        if(this.display_mode) {
            result.display_mode = this.display_mode;
        }
